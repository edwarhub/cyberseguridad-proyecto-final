...
public class PerfilHasUsuarioFacade extends AbstractFacade<PerfilHasUsuario> {
    ...
    public long countAllPerfiles(Integer idusuario) {
        String hql = "SELECT count(obj) FROM PerfilHasUsuario obj WHERE obj.idusuario = " + idusuario;
        return numFromHQL(hql, new Long(0));
    }

    public List<PerfilHasUsuario> listAllPerfiles(Integer idusuario, String attrOrd, String ascDesc, int firstReg, int maxReg) {
        String hql = "SELECT obj FROM PerfilHasUsuario obj WHERE obj.idusuario = " + idusuario + " ORDER BY obj.idpu " + ascDesc;
        return findList(hql, firstReg, maxReg);
    }

    public long countAllUsuarios(Integer idperfil) {
        String hql = "SELECT count(obj) FROM PerfilHasUsuario obj WHERE obj.idperfil = " + idperfil;
        return numFromHQL(hql, new Long(0));
    }

    public List<PerfilHasUsuario> listAllUsuarios(Integer idperfil, String attrOrd, String ascDesc, int firstReg, int maxReg) {
        String hql = "SELECT obj FROM PerfilHasUsuario obj WHERE obj.idperfil = " + idperfil + " ORDER BY obj.idpu " + ascDesc;
        return findList(hql, firstReg, maxReg);
    }
    ...
    public void nuevo(PerfilHasUsuario obj, StringBuilder err) {
        ...
        String sql0 = " SELECT count(*) FROM perfil_has_usuario  WHERE idusuario = " + obj.getIdusuario().getIdusuario()
        ...
        if (m > 0) {
            err.append("No se puede crear por que  existe un registro creado");
            return;
        }
        beginTransaction();
        try {
